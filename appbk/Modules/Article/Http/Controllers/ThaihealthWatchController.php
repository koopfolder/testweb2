...
class ThaihealthWatchController extends Controller
{
    public function getIndex()
    {
        //dd("News");
        $items = Article::Data(['status'=>['publish','draft'],'page_layout'=>'thaihealth_watch']);
        //dd($items);
        return view('article::backend.thaihealth-watch.index', compact('items'));
    }

    public function getIndexiframe()
    {
        $items = Article::Data(['status'=>['publish'],'page_layout'=>'thaihealth_watch']);
    ...
    public function getCreate()
    {
        ...
        return view('article::backend.thaihealth-watch.create',compact('tags'));
    }
    ...
    public function postCreate(CreateRequest $request)
    {
        ...
        $data['page_layout'] = 'thaihealth_watch';
        ...
        self::postLogs(['event'=>'เพิ่มข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'29']);
        return redirect()->route('admin.thaihealth-watch.index')
                            ->with('status', 'success')
    ...
    public function getEdit($id)
    {
        ...
        //dd($data,$revisions);
        return view('article::backend.thaihealth-watch.edit', compact('data','revisions','tags','tags_select'));
    }
    ...
    public function postEdit(EditRequest $request, $id)
    {
        ...
        self::postLogs(['event'=>'แก้ไขข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'29']);
        return redirect()->route('admin.thaihealth-watch.index')
                            ->with('status', 'success')
    ...
    public static function getDataThaihealthWatchPointsToWatchArticle($params)
    {
        $page = (isset($params['page']) ? $params['page']:'1');
        $time_cache  =  ThrcHelpers::time_cache(5);
        $article = '';
        if (Cache::has('data_points_to_watch_article_'.$page)){
            $article = Cache::get('data_points_to_watch_article_'.$page);
        }else{
            $article = Article::FrontList(['page_layout'=>'points_to_watch_article','page'=>$page]);
            //$article = ViewArticlesResearch::FrontList(['page'=>$page]);
            Cache::put('data_points_to_watch_article_'.$page,$article,$time_cache);
            $article = Cache::get('data_points_to_watch_article_'.$page);
        }
    ...
    public static function getDataThaihealthWatchPointsToWatchVideo($params)
    {
        $page = (isset($params['page']) ? $params['page']:'1');
        $time_cache  =  ThrcHelpers::time_cache(5);
        $article = '';
        if (Cache::has('data_points_to_watch_video_'.$page)){
            $article = Cache::get('data_points_to_watch_video_'.$page);
        }else{
            $article = Article::FrontList(['page_layout'=>'points_to_watch_video','page'=>$page]);
            //$article = ViewArticlesResearch::FrontList(['page'=>$page]);
            Cache::put('data_points_to_watch_video_'.$page,$article,$time_cache);
            $article = Cache::get('data_points_to_watch_video_'.$page);
        }
    ...
    public static function getDataThaihealthWatchPointsToWatchGallery($params)
    {
        $page = (isset($params['page']) ? $params['page']:'1');
        $time_cache  =  ThrcHelpers::time_cache(5);
        $article = '';
        if (Cache::has('data_points_to_watch_gallery_'.$page)){
            $article = Cache::get('data_points_to_watch_gallery_'.$page);
        }else{
            $article = Article::FrontList(['page_layout'=>'points_to_watch_gallery','page'=>$page]);
            //$article = ViewArticlesResearch::FrontList(['page'=>$page]);
            Cache::put('data_points_to_watch_gallery_'.$page,$article,$time_cache);
            $article = Cache::get('data_points_to_watch_gallery_'.$page);
        }
