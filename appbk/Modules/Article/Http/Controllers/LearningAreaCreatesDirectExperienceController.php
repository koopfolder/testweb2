...
class LearningAreaCreatesDirectExperienceController extends Controller
{
    public function getIndex()
    {
        //dd("News");
        $items = Article::Data(['status'=>['publish','draft'],'page_layout'=>'learning_area_creates_direct_experience']);
        //dd($items);
        return view('article::backend.learning-area-creates-direct-experience.index', compact('items'));
    }

    public function getIndexiframe()
    {
        $items = Article::Data(['status'=>['publish'],'page_layout'=>'learning_area_creates_direct_experience']);
    ...
    public function getCreate()
    {
        ...
        return view('article::backend.learning-area-creates-direct-experience.create',compact('tags'));
    }
    ...
    public function postCreate(CreateRequest $request)
    {
        ...
        $data['page_layout'] = 'learning_area_creates_direct_experience';
        ...
        self::postLogs(['event'=>'เพิ่มข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'30']);
        return redirect()->route('admin.learning-area-creates-direct-experience.index')
                            ->with('status', 'success')
    ...
    public function getEdit($id)
    {
        ...
        //dd($data,$revisions);
        return view('article::backend.learning-area-creates-direct-experience.edit', compact('data','revisions','tags','tags_select'));
    }
    ...
    public function postEdit(EditRequest $request, $id)
    {
        ...
        self::postLogs(['event'=>'แก้ไขข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'30']);
        return redirect()->route('admin.learning-area-creates-direct-experience.index')
                            ->with('status', 'success')
    ...
    public static function getDataLearningAreaCreatesDirectExperience($params)
    {
        ...
        $article = '';
        if (Cache::has('data_learning_area_creates_direct_experience_page_'.$page)){
            $article = Cache::get('data_learning_area_creates_direct_experience_page_'.$page);
        }else{
            $article = Article::FrontList(['page_layout'=>'learning_area_creates_direct_experience','page'=>$page]);
            //$article = ViewArticlesResearch::FrontList(['page'=>$page]);
            Cache::put('data_learning_area_creates_direct_experience_page_'.$page,$article,$time_cache);
            $article = Cache::get('data_learning_area_creates_direct_experience_page_'.$page);
        }
