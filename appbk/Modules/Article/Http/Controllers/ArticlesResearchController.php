...
class ArticlesResearchController extends Controller
{
    public function getIndex()
    {
        //dd("articles-research");
        $items = Article::Data(['status'=>['publish','draft'],'page_layout'=>'articles_research']);
        //dd($items);
        return view('article::backend.articles-research.index', compact('items'));
    }

    public function getIndexiframe()
    {
        $items = Article::Data(['status'=>['publish'],'page_layout'=>'articles_research']);
        return view('article::index-articles-research-iframe', compact('items'));
    }

    public function getCreate()
    {
        $tags = Tags::Data(['status'=>['publish']])->pluck('title','title');
        return view('article::backend.articles-research.create',compact('tags'));
    }
    ...
    public function postCreate(CreateRequest $request)
    {
        ...
        $data['page_layout'] = 'articles_research';
        ...
        self::postLogs(['event'=>'เพิ่มข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'27']);
        return redirect()->route('admin.articles-research.index')
                            ->with('status', 'success')
    ...
    public function getEdit($id)
    {
        ...
        //dd($data,$revisions);
        return view('article::backend.articles-research.edit', compact('data','revisions','tags','tags_select'));
    }
    ...
    public function postEdit(EditRequest $request, $id)
    {
        ...
        self::postLogs(['event'=>'แก้ไขข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'27']);
        return redirect()->route('admin.articles-research.index')
                            ->with('status', 'success')
    ...
    public static function getDataArticlesResearch($params)
    {
        ...
        $article = '';
        if (Cache::has('data_articles_research_page_'.$page)){
            $article = Cache::get('data_articles_research_page_'.$page);
        }else{
            $article = Article::FrontList(['page_layout'=>'articles_research','page'=>$page]);
            //$article = ViewArticlesResearch::FrontList(['page'=>$page]);
            Cache::put('data_articles_research_page_'.$page,$article,$time_cache);
            $article = Cache::get('data_articles_research_page_'.$page);
        }
    ...
    public static function getListArticlesResearch(){
        //dd("getListArticlesResearch");
        return view('template.list_articles_research_case_front');
    }
