...
class ElearningController extends Controller
{
    ...
    {
        //dd("e-learning");
        $items = Article::Data(['status'=>['publish','draft'],'page_layout'=>'e-learning']);
        //dd($items);
        return view('article::backend.e-learning.index', compact('items'));
    }

    public function getIndexiframe()
    {
        $items = Article::Data(['status'=>['publish'],'page_layout'=>'e-learning']);
        return view('article::index-e-learning-iframe', compact('items'));
    }

    public function getCreate()
    {
        $tags = Tags::Data(['status'=>['publish']])->pluck('title','title');
        return view('article::backend.e-learning.create',compact('tags'));
    }
    ...
    public function postCreate(CreateRequest $request)
    {
        ...
        self::postLogs(['event'=>'เพิ่มข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'6']);
        return redirect()->route('admin.e-learning.index')
                            ->with('status', 'success')
    ...
    public function getEdit($id)
    {
        ...
        //dd($data,$revisions);
        return view('article::backend.e-learning.edit', compact('data','revisions','tags','tags_select'));
    }
    ...
    public function postEdit(EditRequest $request, $id)
    {
        ...
        self::postLogs(['event'=>'แก้ไขข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'6']);
        return redirect()->route('admin.e-learning.index')
                            ->with('status', 'success')
    ...
    public static function getDataElearing($params)
    {
        $article = Article::FrontList(['page_layout'=>'e-learning','page'=>(isset($params['page']) ? $params['page']:'1')]);
        //dd($article);
        //dd($article->count());
        $data = array();
        $data['items'] = $article;
