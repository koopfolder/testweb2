...
namespace App\Modules\Chairmanstatement\Models;
...
class Article extends Model implements HasMediaConversions
{
    ...
    public function scopeData($query,$params)
    {
        ...
        return $query->whereIn('status',$params['status'])->where('page_layout','=',$params['page_layout'])->with('createdBy','updatedBy')->orderBy('id','desc')->limit(1)->get();
    }
    ...
    public function scopeFrontData($query,$params)
    {
        ...
        // }
            $query->select('id','title_th','description_th','title_en','description_en');
        return $query
                     ->where('status','=','publish')
                     ->where('page_layout','=',$params)
                     ->first();
    }
