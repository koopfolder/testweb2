...
@if(method_exists('App\Modules\Exhibition\Http\Controllers\FrontendController', 'getDataListExhibition'))   
    @php
        $data = App\Modules\Exhibition\Http\Controllers\FrontendController::getDataListExhibition(['page_layout'=>$page_layout,'title'=>$title]);  
        //dd($data['items']->count());
    @endphp
@endif
@extends('layouts.app')
        ...
        <div class="container">
            ...
            <div class="row">
                <div class="col-xs-12 wrap_btn_viewmore">
                    {!! isset($data['page'])?$data['page']->appends(\Input::all())->render():'' !!}  
                </div>
            </div>
        </div>
</section>
