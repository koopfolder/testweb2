...
@php
	$lang = \App::getLocale();
    $layout_params = json_decode($menu['layout_params']);
    //dd($layout_params);
    $category_id = \Hashids::encode($layout_params->id);
    //dd($category_id);
@endphp
@if(method_exists('App\Modules\Article\Http\Controllers\HealthLiteracyController', 'getDataHealthLiteracy'))   
    @php

        $data = App\Modules\Article\Http\Controllers\HealthLiteracyController::getDataHealthLiteracy($request->all(),$category_id);  
       //dd($data['items']);
    @endphp
@endif
@extends('layouts.app')
@section('content')
@include('partials.search_health_literacy')
