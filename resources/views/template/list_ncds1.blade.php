...
@php
    ...
    //dd("asdassds");
@endphp
@if(method_exists('App\Modules\Api\Http\Controllers\FrontendController', 'getDataNcds1'))   
    @php
        $data = App\Modules\Api\Http\Controllers\FrontendController::getDataNcds1($request->all());  
        //dd($data['items']);
    @endphp
@endif
@extends('layouts.app')
@section('content')
@include('partials.search_ncds1')
            ...
            <div class="row row_learning">
                ...
                @if($data['items']->count())
                    @foreach($data['items'] AS $key=>$value)
                        @php
                           //dd($value);
                            $value->url = route('article-ncds1-detail',$value->slug);
...
@section('meta')
    @parent
    <title>รู้จัก NCDs</title>
    <meta charset="UTF-8">
    <meta name="description" content="รู้จัก NCDs">
    <meta name="keywords" content="รู้จัก NCDs">
