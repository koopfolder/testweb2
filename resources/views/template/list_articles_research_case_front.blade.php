...
@php
	//dd("Test");
@endphp
@if(method_exists('App\Modules\Article\Http\Controllers\ArticlesResearchController', 'getDataArticlesResearch')) 
    @php
        $data = App\Modules\Article\Http\Controllers\ArticlesResearchController::getDataArticlesResearch($request->all());  
        //dd($data,$request->all());
    @endphp
@endif
@extends('layouts.app')
            ...
            <div class="row row_learning">
                ...
                @if($data['items']->count())
                    @foreach($data['items'] AS $key=>$value)
                        ...
                        <a href="{{ $value->url }}" onclick="clickview('view_item','{{$value['id']}}','{{($value['tags']!= null)?$value['tags']:'null'}}','{{($value['category_id']!= null)?$value['category_id']:'null'}}')">
                                <figure>
...
@section('meta')
    @parent
    <title>บทความ / งานวิจัย</title>
    <meta charset="UTF-8">
    <meta name="description" content="บทความ / งานวิจัย">
    <meta name="keywords" content="บทความ / งานวิจัย">
