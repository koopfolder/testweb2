...
@php
    $lang = \App::getLocale();
@endphp
@if(method_exists('App\Modules\Article\Http\Controllers\NewsController', 'getDataListNews'))   
    @php
        $data = App\Modules\Article\Http\Controllers\NewsController::getDataListNews($request->all());  
        //dd($data['items']->count());
    @endphp
@endif
@extends('layouts.app')
                    ...
                    @if($data['items']->count())
                        @foreach($data['items'] AS $key=>$value)
                            @php
                                //dd($menu);
                                //dd($value);
                                $value->url = route('news-event-detail',$value->slug);
