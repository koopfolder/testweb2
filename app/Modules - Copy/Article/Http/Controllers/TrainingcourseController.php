...
class TrainingcourseController extends Controller
{
    ...
    {
        //dd("training-course");
        $items = Article::Data(['status'=>['publish','draft'],'page_layout'=>'training_course']);
        //dd($items);
        return view('article::backend.training-course.index', compact('items'));
    }

    public function getIndexiframe()
    {
        $items = Article::Data(['status'=>['publish'],'page_layout'=>'training_course']);
        return view('article::index-trainingcourse-iframe', compact('items'));
    }

    public function getCreate()
    {
        $tags = Tags::Data(['status'=>['publish']])->pluck('title','title');
        return view('article::backend.training-course.create',compact('tags'));
    }
    ...
    public function postCreate(CreateRequest $request)
    {
        ...
        $data['page_layout'] = 'training_course';
        ...
        self::postLogs(['event'=>'เพิ่มข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'5']);
        return redirect()->route('admin.training-course.index')
                            ->with('status', 'success')
    ...
    public function getEdit($id)
    {
        ...
        //dd($data,$revisions);
        return view('article::backend.training-course.edit', compact('data','revisions','tags','tags_select'));
    }
    ...
    public function postEdit(EditRequest $request, $id)
    {
        ...
        self::postLogs(['event'=>'แก้ไขข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'5']);
        return redirect()->route('admin.training-course.index')
                            ->with('status', 'success')
    ...
    public static function getDataTrainingCourse($params)
    {
        $article = Article::FrontList(['page_layout'=>'training_course','page'=>(isset($params['page']) ? $params['page']:'1')]);
        //dd($article);
        //dd($article->count());
        $data = array();
        $data['items'] = $article;
