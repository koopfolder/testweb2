...
class ListMedia extends Model implements HasMediaConversions
{
    ...
    public function scopeData2($query,$params)
    {
        ...
        $query->select('id','title','recommend','articles_research','include_statistics','notable_books','interesting_issues','featured','json_data','created_at','created_by','updated_by','status','knowledges','media_campaign');
    ...
    public function scopeDetail($query,$params){
        $lang = $this->getLang();
        $query->select('id','title','description','interesting_issues','hit','json_data','template','featured','tags','created_at','created_by','updated_by','status');
