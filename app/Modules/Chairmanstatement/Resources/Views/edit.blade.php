...
<section class="content-header">
    <h1>{{ trans('chairmanstatement::backend.chairmanstatement') }}</h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('admin.dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('chairmanstatement::backend.home') }}</a></li>
        <li><a href="{{ route('admin.chairmanstatement.index') }}">{{ trans('chairmanstatement::backend.chairmanstatement') }}</a></li>
        <li class="active">{{ trans('chairmanstatement::backend.edit') }}</li>
    </ol>
</section>

<section class="content">
    <div class="row">
        {{ Form::open(['url' => route('admin.chairmanstatement.edit', $data->id), 'files' => true]) }}
        <div class="col-md-9">
            <div class="row">
                <div class="col-md-12">
                    <div class="nav-tabs-custom">
                        <ul class="nav nav-tabs">
                            <li class="active"><a href="#tab_description" data-toggle="tab">{{ trans('chairmanstatement::backend.description') }}</a></li>
                            <li><a href="#tab_revisions" data-toggle="tab">{{ trans('chairmanstatement::backend.revisions') }}</a></li>
                        </ul>

                        <div class="tab-content">
                            <div class="tab-pane active" id="tab_description">
                                <div class="form-group">
                                    <label for="InputName">{{ trans('chairmanstatement::backend.title_th') }} <span style="color:red">*</span></label>
                                    {{ Form::text('title_th',$data->title_th ?? old('title_th'),['class'=>'form-control']) }}
                                </div>
                                <div class="form-group">
                                    <label for="InputName">{{ trans('chairmanstatement::backend.title_en') }}</label>
                                    {{ Form::text('title_en',$data->title_en ?? old('title_en'),['class'=>'form-control']) }}
                                </div>
                                <div class="form-group">
                                    <label for="InputName">{{ trans('chairmanstatement::backend.description_th') }} <span style="color:red">*</span></label>
                                    {{ Form::textarea('description_th',$data->description_th ?? old('description_th'),['class'=>'form-control ckeditor']) }}
                                </div>
                                <div class="form-group">
                                    <label for="InputName">{{ trans('chairmanstatement::backend.description_en') }} </label>
                            ...
                            <div class="tab-pane" id="tab_revisions">
                                @include('chairmanstatement::partials.corporategovpolicy_revision', ['revisions' => $revisions])
                            </div>
                          <!-- //  <a href="" class="btn btn-default"><i class="fa fa-arrow-left"></i> {{ trans('chairmanstatement::backend.back') }}</a> -->
                            <a href="{{ route('preview-chairman-statement') }}" class="btn btn-success" target="_blank">
                                <i class="fa fa-eye"></i> Preview
                            </a>
                            <button class="btn btn-success pull-right"><i class="fa fa-floppy-o" aria-hidden="true"></i> {{ trans('chairmanstatement::backend.submit') }}</button>
                        </div>
                    </div>
                </div>
            </div>
