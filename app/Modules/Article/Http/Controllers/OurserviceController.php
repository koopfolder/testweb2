...
class OurserviceController extends Controller
{
    ...
    {
        //dd("OurService");
        $items = Article::Data(['status'=>['publish','draft'],'page_layout'=>'our_service']);
        //dd($items);
        return view('article::backend.ourservice.index', compact('items'));
    }

    public function getIndexiframe()
    {
        $items = Article::Data(['status'=>['publish'],'page_layout'=>'our_service']);
        return view('article::index-ourservice-iframe', compact('items'));
    }

    public function getCreate()
    {
        $tags = Tags::Data(['status'=>['publish']])->pluck('title','title');
        return view('article::backend.ourservice.create',compact('tags'));
    }
    ...
    public function postCreate(CreateRequest $request)
    {
        ...
        self::postLogs(['event'=>'เพิ่มข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'3']);
    ...
    public function getEdit($id)
    {
        ...
        //dd($data,$revisions);
        return view('article::backend.ourservice.edit', compact('data','revisions','tags','tags_select'));
    }
    ...
    public function postEdit(EditRequest $request, $id)
    {
        ...
        self::postLogs(['event'=>'แก้ไขข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'3']);
        return redirect()->route('admin.our-service.index')
                            ->with('status', 'success')
    ...
    public static function getDataOurservice($params)
    {
        $article = Article::FrontList(['page_layout'=>'our_service','page'=>(isset($params['page']) ? $params['page']:'1')]);
        //dd($article);
        //dd($article->count());
        $data = array();
        $data['items'] = $article;
