...
class SooklibraryController extends Controller
{
    public function getIndex()
    {
        //dd("sooklibrary");
        $items = Article::Data(['status'=>['publish','draft'],'page_layout'=>'sook_library']);
        //dd($items);
        return view('article::backend.sooklibrary.index', compact('items'));
    }

    public function getIndexiframe()
    {
        $items = Article::Data(['status'=>['publish'],'page_layout'=>'sook_library']);
        return view('article::index-sook-library-iframe', compact('items'));
    }

    public function getCreate()
    {
        $tags = Tags::Data(['status'=>['publish']])->pluck('title','title');
        return view('article::backend.sooklibrary.create',compact('tags'));
    }
    ...
    public function postCreate(CreateRequest $request)
    {
        ...
        self::postLogs(['event'=>'เพิ่มข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'4']);
        return redirect()->route('admin.sook-library.index')
                            ->with('status', 'success')
    ...
    public function getEdit($id)
    {
        ...
        //dd($data,$revisions);
        return view('article::backend.sooklibrary.edit', compact('data','revisions','tags','tags_select'));
    }
    ...
    public function postEdit(EditRequest $request, $id)
    {
        ...
        self::postLogs(['event'=>'แก้ไขข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'4']);
        return redirect()->route('admin.sook-library.index')
                            ->with('status', 'success')
    ...
    public static function getDataSooklibrary($params)
    {
        $article = Article::FrontList(['page_layout'=>'sook_library','page'=>(isset($params['page']) ? $params['page']:'1')]);
        //dd($article);
        //dd($article->count());
        $data = array();
        $data['items'] = $article;
