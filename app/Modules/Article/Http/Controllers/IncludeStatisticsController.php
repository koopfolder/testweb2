...
use App\Modules\Api\Models\{ListMedia,ListArea,ListCategory,ListIssue,ListProvince,ListSetting,ListTarget,ListMediaIssues,ListMediaKeywords,ListMediaTargets,ViewMedia,ViewArticlesResearch,ViewIncludeStatistics,MediaHitLogs,Tags,DataTags};
...
class IncludeStatisticsController extends Controller
{
    public function getIndex()
    {
        //dd("include-statistics");
        $items = Article::Data(['status'=>['publish','draft'],'page_layout'=>'include_statistics']);
        //dd($items);
        return view('article::backend.include-statistics.index', compact('items'));
    }

    public function getIndexiframe()
    {
        $items = Article::Data(['status'=>['publish'],'page_layout'=>'include_statistics']);
        return view('article::index-include-statistics-iframe', compact('items'));
    }

    public function getCreate()
    {
        $tags = Tags::Data(['status'=>['publish']])->pluck('title','title');
        return view('article::backend.include-statistics.create',compact('tags'));
    }
    ...
    public function postCreate(CreateRequest $request)
    {
        ...
        $data['page_layout'] = 'include_statistics';
        ...
        self::postLogs(['event'=>'เพิ่มข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'8']);
        return redirect()->route('admin.include-statistics.index')
                            ->with('status', 'success')
    ...
    public function getEdit($id)
    {
        ...
        //dd($data,$revisions);
        return view('article::backend.include-statistics.edit', compact('data','revisions','tags','tags_select'));
    }
    ...
    public function postEdit(EditRequest $request, $id)
    {
        ...
        self::postLogs(['event'=>'แก้ไขข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'8']);
        return redirect()->route('admin.include-statistics.index')
                            ->with('status', 'success')
    ...
    public static function getDataIncludeStatistics($params)
    {
        ...
        $article = '';
        if (Cache::has('data_include_statistics_page_'.$page)){
            $article = Cache::get('data_include_statistics_page_'.$page);
        }else{
            $article = Article::FrontList(['page_layout'=>'include_statistics','page'=>$page]);
            //$article = ViewIncludeStatistics::FrontList(['page'=>$page]);
            Cache::put('data_include_statistics_page_'.$page,$article,$time_cache);
            $article = Cache::get('data_include_statistics_page_'.$page);
        }
        $data = array();
        $data['items'] = $article;
    ...
    public static function getListIncludeStatistics(){
        //dd("getListArticlesResearch");
        return view('template.list_include_statistics_case_front');
    }
