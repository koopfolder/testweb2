...
class ThaihealthWatchPointsToWatchArticleController extends Controller
{
    
    public function getIndex()
    {
        //dd("News");
        $items = Article::Data(['status'=>['publish','draft'],'page_layout'=>'points_to_watch_article']);
        //dd($items);
        return view('article::backend.points-to-watch-article.index', compact('items'));
    }

    public function getIndexiframe()
    {
        $items = Article::Data(['status'=>['publish'],'page_layout'=>'points_to_watch_article']);
        return view('article::index-iframe', compact('items'));
    }

    public function getCreate()
    {
        $years = ThrcHelpers::getYearForBackend();
        //dd($year);
        $tags = Tags::Data(['status'=>['publish']])->pluck('title','title');
        return view('article::backend.points-to-watch-article.create',compact('tags','years'));
    }
    ...
    public function postCreate(CreateRequest $request)
    {
        $data = $request->all();
        //dd($data);
        $data['page_layout'] = 'points_to_watch_article';
        ...
        self::postLogs(['event'=>'เพิ่มข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'41']);
        return redirect()->route('admin.thaihealth-watch.points-to-watch-article.index')
                            ->with('status', 'success')
    ...
    public function getEdit($id)
    {
        ...
        $years = ThrcHelpers::getYearForBackend();
        //dd($data,$revisions);
        return view('article::backend.points-to-watch-article.edit', compact('data','revisions','tags','tags_select','years'));
    }
    ...
    public function postEdit(EditRequest $request, $id)
    {
        ...
        self::postLogs(['event'=>'แก้ไขข้อมูลหัวข้อ "'.$data['title'].'"','module_id'=>'41']);
        return redirect()->route('admin.thaihealth-watch.points-to-watch-article.index')
                            ->with('status', 'success')
