...
<section class="content-header">
    <h1>{{ trans('article::backend.edit_form-generator') }}</h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('admin.dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('article::backend.home') }}</a></li>
        <li><a href="{{ route('admin.thaihealth-watch.index') }}">{{ trans('article::backend.thaihealth-watch') }}</a></li>
        <li><a href="{{ route('admin.thaihealth-watch.form-generator.index') }}">{{ trans('article::backend.form-generator') }}</a></li>
...
<section class="content">
    <div class="row">
        {{ Form::open(['url' => route('admin.thaihealth-watch.form-generator.edit', $data->id), 'files' => true]) }}
                    ...
                    <div class="nav-tabs-custom">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane active" id="tab_description">
                                ...
                                <div class="form-group">
                                <label for="InputName">{{ trans('article::backend.form-generator') }} <span style="color:red">*</span></label>
                                    ...
                                    <textarea name="form-generator-data" id="form-generator-data" style="display:none">{{ $data['dol_json_data'] }}</textarea>
                                    <a id="edit-form" class="btn btn-default">Edit Form</a>
                            ...
                            <a href="{{ route('admin.thaihealth-watch.form-generator.index') }}" class="btn btn-default"><i class="fa fa-arrow-left"></i> {{ trans('article::backend.back') }}</a>
...
<script type="text/javascript">
    ...
    jQuery($ => {
        var options = {
            ...
            $('#form-generator-data').val(formData);
            toggleEdit();
        ...
        fb.promise.then(function(fb) {
            var formData = '<?php echo $data['dol_json_data']; ?>';
            //console.log(formData,typeof formData);        
            fb.actions.setData(formData);
        });

        // fbTemplate.actions.setLang('th-TH');
        // $('#setLanguage').change(function() {
        //     var lang = $(this).val();
        //     console.log(lang);
        //     fbTemplate.actions.setLang(lang)
        // });    
    });
    ...
    function del_gallery(id){
        ...
        if(media_id !=''){
            jQuery.ajax({
                url: '{{ route("admin.thaihealth-watch.ajaxdeletegallery") }}',
                type: "POST",
                dataType: 'json',
    ...
    function del_document(id){
        ...
        if(media_id !=''){
            jQuery.ajax({
                url: '{{ route("admin.thaihealth-watch.ajaxdeletedocument") }}',
                type: "POST",
                dataType: 'json',
                data: {_token:jQuery('meta[name="csrf-token"]').attr('content'),id:media_id},
                success:function(response){
