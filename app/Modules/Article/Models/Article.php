...
class Article extends Model implements HasMediaConversions
{
    ...
    public function scopeData($query,$params)
    {
        ...
        // }
            $query->select('id','title','year','slug','description','created_at','created_by','updated_by','status','page_layout');
    ...
    public function scopeFrontDataInterestingIssuesNews($query,$params)
    {
        ...
        $query->select('id','title','short_description AS description','slug','dol_cover_image','tags');
        if($params['featured'] ===true){
                $query->where('featured','=',2);
        }else{
                //$query->where('featured','!=',2);
    ...
    public function scopeFrontList($query,$params){
        ...
        $query->select('id','title','short_description AS description','slug','hit','created_at','status','featured','updated_at','dol_cover_image','tags','category_id');
    ...
    public function scopeFrontListRelated($query,$params){
        ...
        //dd($params);
        $date = date('Y-m-d H:i:s');
        $query->selectRaw('id,title,short_description AS description,slug,hit,created_at,status,dol_json_data AS json_data,CONCAT("article") AS data_type,tags,category_id');
        $query->where('status','=','publish');
        $query->where('page_layout','=',$params['page_layout']);
    ...
    public function scopeFrontListRelated2($query,$params){
        ...
        //dd($params);
        $date = date('Y-m-d H:i:s');
        $query->selectRaw('id,title,short_description AS description,slug,hit,created_at,status,CONCAT("article") AS data_type,tags');
        $query->where('status','=','publish');
    ...
    public function scopeFrontListRecommend($query,$params){
        ...
        //dd($params);
        $date = date('Y-m-d H:i:s');
        $query->select('id','title','short_description AS description','slug','hit','created_at','status','tags','category_id');
        $query->where('status','=','publish');
        $query->where('page_layout','=',$params['page_layout']);
    ...
    public function scopeDetail($query,$params){
        $lang = $this->getLang();
        $query->select('id','title','description','hit','created_at','updated_at','status','meta_title','meta_keywords','meta_description','page_layout','dol_url','dol_json_data','dol_UploadFileID','dol_template','category_id','slug','tags','video_path',\DB::raw("JSON_UNQUOTE(JSON_EXTRACT(dol_json_data,'$.Issues[*].Name')) as Issues"),\DB::raw("JSON_UNQUOTE(JSON_EXTRACT(dol_json_data,'$.Targets[*].Name')) as Targets"),\DB::raw("JSON_UNQUOTE(JSON_EXTRACT(dol_json_data,'$.Settings[*].Name')) as Settings"));
