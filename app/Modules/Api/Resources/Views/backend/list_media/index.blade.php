				...
				<div class="box-body">
					...
					<div class="row">
						<div class="col-xs-6">
							...
							<a href="{{ route('DonwLoadExcelReport') }}" class="btn btn-success">
						...
						@isset($old['status'])
							...
							<button class="btn btn-danger" type="button" onclick="recycletrashall()">	
										<i class="fa fa-recycle"></i> กู้คืนสื่อจากถังขยะทั้งหมด
							</button>
							<button class="btn btn-danger" type="button" onclick="delecttrashall()">
										<i class="fa fa-trash"></i> ลบสื่อออกถาวรทั้งหมด
							</button>
							</div>
							@endif
						@endisset
						...
						<tbody>
							...
							<tr>
								...
								<th id="SendMediaTermStatus_{{ $item->id }}">
									...
									@elseif($item->SendMediaTermStatus == 49)

									<a href="javascript:void(0);" >
									<span class="label" style="background-color: #C39200;">
										  รอ
									</span>
								...
								<th>
									...
									<div class="box-body">
										...
										<div class="form-group">
											...
											@php 
													...
													$target_array = collect($target2)->pluck('title','target_id','TargetGuoupID');
													$target_select = [];
										...
										<div class="coupon">
											...
											<div class="form-group MediaStatus">
												<label for="InputName">{{ trans('api::backend.status') }} </label>
												{{ Form::radio('status_'.$item->id, 'publish', ($item->status == 'publish' ? true:false), ['onclick'=>'hideReason("not_publish_reason_' . $item->id . '", "publish");','class' => 'status_chack']) }} {{ trans('api::backend.publish') }}
												{{ Form::radio('status_'.$item->id, 'draft', ($item->status == 'draft' ? true:false), ['onclick'=>'hideReason("not_publish_reason_' . $item->id . '", "draft");','class' => 'status_chack']) }} {{ trans('api::backend.draft') }}
											</div>
											<!-- <div class="form-group fg_1" id="not_publish_reason_{{ $item->id }}" {{ ($item->status == 'publish') ? 'style=display:none;' : '' }}>
												...
												<select name="not_publish_reason_{{ $item->id }}" id="" class="form-control js-tags-tokenizer sg_1" data-placeholder="เลือกเหตุผล" multiple="multiple">
													<option value="1" {{ (in_array("1", $tmp_not_publish_reason)) ? "selected" : ""  }} >รายงานโครงการ/ รายงานงวด/ ภาพกิจกรรมในโครงการฯ/ ภาพข่าวในโครงการฯ</option>
													<option value="2" {{ (in_array("2", $tmp_not_publish_reason)) ? "selected" : ""  }} >ข่าวที่ตัดปะ/ โควทคำ/ โควทข่าว/ โควทภาพ หรือข่าวจากแหล่งต่างๆ</option>
													<option value="3" {{ (in_array("3", $tmp_not_publish_reason)) ? "selected" : ""  }} >รายงานอื่นๆ ของสำนักหรือของภาคี เช่น รายงานการดำเนินงาน รายงานประจำปี รายงานสรุปผลโครงการ ฯลฯ</option>
													<option value="4" {{ (in_array("4", $tmp_not_publish_reason)) ? "selected" : ""  }} >เป็นสื่อที่ใช้ในการขับเคลื่อนงานในเชิงพื้นที่ที่ เหมาะสำหรับผู้นำการเปลี่ยนแปลง </option>
													<option value="5" {{ (in_array("5", $tmp_not_publish_reason)) ? "selected" : ""  }} >ตัวอักษรเล็ก ภาพไม่ชัดเจน (ส่วนใหญ่เป็นสื่อ Infographic)</option>
													<option value="6" {{ (in_array("6", $tmp_not_publish_reason)) ? "selected" : ""  }} >การตั้งชื่อไม่สอดคล้องกับตัวสื่อ ขอให้ใส่ชื่อให้สมบูรณ์ (ไม่ควรใช้ชื่อไฟล์สื่อเป็นชื่อสื่อ) ขอให้ปรับชื่อของสื่อใน DOL </option>
													<option value="7" {{ (in_array("7", $tmp_not_publish_reason)) ? "selected" : ""  }} >ไฟล์ที่เป็นการออกแบบ หรือโครงสร้าง หรือภาพถ่าย หรือ logo ขอให้ใส่สื่อใหม่เป็น Jpeg หรือ PNG ใน DOL (สำนักต้องทำขั้นตอนใน DOL ใหม่) </option>
													<option value="8" {{ (in_array("8", $tmp_not_publish_reason)) ? "selected" : ""  }} >ปลายทาง link URL จำกัดสิทธิ์เข้าถึง หรือมีการโยกย้ายปรับปรุงที่ส่งผลกระทบกับตัวสื่อ ทำให้หาสื่อไม่เจอ </option>
													<option value="9" {{ (in_array("9", $tmp_not_publish_reason)) ? "selected" : ""  }} >ข้อมูลไม่สมบูรณ์ขาดหน้าปกสื่อ ขอให้เพิ่มเติมการใส่รูปภาพหน้าปกสื่อที่เป็น Link , คลิป, ไฟล์เสียง, Multimedia ต่างๆ ใน DOL</option>
												</select>
											</div> -->

											@if ($item->SendMediaTermStatus == '50' ||$item->SendMediaTermStatus == '49' ) 
												...
												<div class="form-group">
													...
													{{ Form::radio('term_'.$item->id, '52', ($item->SendMediaTermStatus == '52' ? true:false), ['onclick'=>'hideReason("not_term_' . $item->id . '", "draft");']) }} ไม่เป็นสื่อวาระกลาง
													{{ Form::radio('term_'.$item->id, '49', ($item->SendMediaTermStatus == '49' ? true:false),["hidden"], ['onclick'=>'hideReason("not_term_' . $item->id . '", "wait");']) }} 
					
												</div>
												<div class="form-group" id="not_term_{{ $item->id }}"  {{ ($item->SendMediaTermStatus == null) ? '' : 'style=display:none;' }}>
											...
											<div class="form-group">
												...
												{{ Form::radio('media_campaign_'.$item->id, '2', ($item->media_campaign == '2' ? true:false), ['onclick'=>'hideReason("campaign_not_publish_reason_' . $item->id . '", "publish")','class' => 'media_campaign_chack']) }}{{ trans('api::backend.publish') }}
												{{ Form::radio('media_campaign_'.$item->id, '1', ($item->media_campaign == '1' ? true:false), ['onclick'=>'hideReason("campaign_not_publish_reason_' . $item->id . '", "draft")','class' => 'media_campaign_chack']) }}{{ trans('api::backend.draft') }}
											</div>
							
											<div class="form-group" id="show_index" style="@if(!empty($item->media_campaign) && $item->media_campaign == 1 || ($item->status == 'draft') || ($json_data->Template != 'Multimedia')) display:none; @endif"> 
											<label for="InputName"></label>
											<input type="checkbox" class="chack_show_data" id="chack_show_data" name="chack_show_data" value="1" >
												ให้สื่อรณรงค์นี้แสดงหน้าแรก
											</div>	
											...
											<div class="form-group">
												<label for="InputName"> 
														<input type="checkbox" class="chk_dol" onclick="perso({{$item->id}})" name="show_dol{{$item->id}}" value="2" {{$item->show_dol == '2' ? 'checked':''}} >
														Persona Health 
												</label>
												<!-- <div class="chk_data_status" style="display: contents;">
													<input type="checkbox" name="api_{{$item->id}}"     value="publish" {{$item->api == 'publish' ? 'checked':''}}>Api &nbsp;&nbsp;&nbsp;&nbsp;
													<input type="checkbox" name="web_view_{{$item->id}}" value="1" {{$item->web_view == '1' ? 'checked':''}} > Webview
												</div> -->
											</div>
											...
											<div class="form-group">
												...
												<input id="set_start_date_{{$item->id}}"  class="showtime_date start_date_chk" type="date" name="{{'start_date'.$item->id}}" style="margin-left:0;" placeholder="{{ trans('dashboard::backend.start_date') }}" class="form-control" value="{{$item->start_date}}"/><br>
												<label for="InputName">
													{{ Form::checkbox('chk_end_date','1', ($item->end_date != '' ? true : false), ['onclick'=>'setReadOnly("set_end_date_' . $item->id . '");']) }} วันที่สิ้นสุด 
												</label>
												<input id="set_end_date_{{$item->id}}" data-name="{{$item->id}}" class="showtime_date end_date_chk" type="date" name="{{'end_date'.$item->id}}" class="form-control" value="{{$item->end_date}}" placeholder="{{ trans('dashboard::backend.start_date') }}" {{ ($item->end_date != '' ? '' : 'readonly') }} />
											</div>
											<div class="form-group">
										...
										<div class="coupon">
											...
											<div class="form-group">
												...
												{{ Form::radio('ncds_2_situation'.$item->id, '0', ($item->ncds_2_situation == '0' ? true:false)) }} ไม่เผยแพร่
											</div>
									...
									@if (is_null($item->media_trash))
									<a href="#ex{{ $item->id }}" onclick="chk({{ $item->id }})" rel="modal:open" class="btn btn-primary">
									...
									@php
										$title_trash=str_replace("'",'"',$item->title);
									@endphp

									@if (is_null($item->media_trash))
								
									<button class="btn btn-danger" onclick="movetotrash('{{$item->id}}','{{$title_trash}}')">
										<i class="fa fa-trash"></i> ย้ายไปที่ถังขยะ
									</button>
									@else
									<button class="btn btn-danger" onclick="recycletrash('{{$item->id}}','{{$title_trash}}')">
										<i class="fa fa-recycle"></i> กู้คืนสื่อจากถังขยะ
									</button>
									<button class="btn btn-danger" onclick="delecttrash('{{$item->id}}','{{$title_trash}}')">
									...
									@if (!is_null($item->local_path))
										<button class="btn btn-danger" onclick="deletemediafile('{{$item->id}}','{{$title_trash}}')">
...
function movetotrash(id,title) {
	...
	Swal.fire({
		icon: 'question',
		title: "ท่านต้องการย้ายสื่อ : <span>" + title +"</span>"+ "\nไปไว้ที่ถังขยะใช่หรือไม่ ?",   
...
function recycletrashall() {
	Swal.fire({
		icon: 'question',
		title: "ท่านต้องการกู้คืนสื่อทั้งหมดใช่หรือไม่?",   
		showCancelButton: true,
		confirmButtonColor: '#154734',
		cancelButtonColor: '#d33',
		confirmButtonText:  "ใช่ต้องการ",
...
function recycletrash(id,title) {
	Swal.fire({
		icon: 'question',
		title: "ท่านต้องการกู้คืนสื่อ : <span>" + title +"</span>"+ "\ใช่หรือไม่ ?",   
		showCancelButton: true,
		confirmButtonColor: '#154734',
		cancelButtonColor: '#d33',
		confirmButtonText:  "ใช่ต้องการ",
...
function deletemediafile(id,title) {
	Swal.fire({
		icon: 'question',
		title: "ท่านต้องการลบไฟล์สื่อ : <span>" + title +"</span>"+ "\ใช่หรือไม่ ?",   
		showCancelButton: true,
		confirmButtonColor: '#154734',
		cancelButtonColor: '#d33',
		confirmButtonText:  "ใช่ต้องการ",
...
function delecttrash(id,title) {
	Swal.fire({
		icon: 'question',
		title: "ท่านต้องการลบสื่อ : <span>" + title +"</span>"+ "\ถาวรใช่หรือไม่ ?",   
		showCancelButton: true,
		confirmButtonColor: '#154734',
		cancelButtonColor: '#d33',
		confirmButtonText:  "ใช่ต้องการ",
...
function delecttrashall() {
	Swal.fire({
		icon: 'question',
		title: "ท่านต้องการลบสื่อทั้งหมดใช่หรือไม่ ?",   
		showCancelButton: true,
		confirmButtonColor: '#154734',
		cancelButtonColor: '#d33',
		confirmButtonText:  "ใช่ต้องการ",
...
function transfer(id){
	 var chack_show_data = $(`#ex${id}`).find('.chack_show_data').prop('checked');
if (chack_show_data) {
	chack_show_data = 1
} else {
	chack_show_data = 0
...
let issue_chack = $('#issue_'+id).val();

	if(issue_chack.length == 1){
		if(issue_chack[0] == 40){
			Swal.fire({
					icon: 'error',
					title: 'ไม่สามารถเลือกเฉพาะ อื่นๆ ได้'
				});
			return false
		}
	}
...
// return
	...
	// else if(tags_dol.length<5){
	// 	Swal.fire({
	// 				icon: 'error',
	// 				title: 'กรุณาเพิ่มแท็ก (Dol) ขั้นต่ำ 5 แท็ก'
	// 			});
	// 	$('.content').loading('toggle');
	// 	return false
	// }
	...
	let arr_target = [];
	let test = $( `#target_${id} option:selected` ).each(function(){
		let data = $(this).text()
		data = data.substr(data.lastIndexOf("(") + 1)
		arr_target.push(data.split(')')[0])
	});
		if(arr_target.includes("กลุ่มเพศ") == false || arr_target.includes("กลุ่มอายุ") == false){
			Swal.fire({
					icon: 'error',
					title: 'กรุณาเลือก Targets<br>กลุ่มเพศและอายุอย่างน้อย 1 รายการ'
				});
			$('.content').loading('toggle');
			return false
	}
	...
	formData.append('chack_show_data',chack_show_data); 
	console.log('showdata',chack_show_data)
	...
	Swal.fire({
					icon: 'question',
					title: 'อาจใช้เวลาในการโหลดสื่อสักครู่',
	  ...
	  .then(function (response) {
	    ...
	    if(response.status === 200){
			...
			let icon = 'success';
				...
				//warning
			console.log(response.data.data_Description);
			if(response.data.data_Description.Code === "00"){
				location.reload();
			}
			const data = JSON.parse(response.data.data_Description);
			//console.log(2);
			if(data.Code =="02"){
				icon = 'success';
				message = 'บันทึกบน THRC สำเร็จแต่ไม่สามารถบันทึกบน DOL ได้เนื่องจาก : <br>'+data.Description;
			}
			$('.content').loading('toggle');
			...
			switch(response.data.field) {
				...
				case 'SendMediaTermStatus':
			    	if(response.data.status_data === true){
			    		...
			    		$("#SendMediaTermStatus_"+id+' a span').attr('class', 'label label-gray');
...
function chk(id) {
	var chk_dol = $("input[name=show_dol"+id+"]:checked").val();
	if(chk_dol=='2'){
		$(".chk_data_status").show();
	}else{
		$(".chk_data_status").hide();
	}
}
function perso(id){
	var chk_dol = $("input[name=show_dol"+id+"]:checked").val();
...
$(document).ready(function() {
	...
	var chk_dol = $('#chk_dol').val();

	$('.end_date_chk').on('change', function(e) {
					var id = $(this).data('name');
					var start_date = $('#set_start_date_'+id).val();
					var end_data = $('#set_end_date_'+id).val();
							
			
					if (start_date=='') {
						Swal.fire({
							icon: 'error',
							title: 'กรุณากรอกวันที่เผยแพร่',
							showConfirmButton: true,
							confirmButtonText: 'OK',
						})
						$('#set_start_date_'+id).val('');
						$('#set_end_date_'+id).val('');
					}else if (start_date >= end_data) {
						Swal.fire({
							icon: 'error',
							title: 'ท่านกรอกวันที่ไม่ถูกต้อง',
							showConfirmButton: true,
							confirmButtonText: 'OK',
						})
						$('#set_start_date_'+id).val('');
	...
	$('.media_campaign_chack').on('change', function(e) {
		var media_campaign = $(this).val();
		let Template_view = $(this).closest('.modal').find("input[name=Template_view]").val();
		var status_chack =$(this).closest('.modal').find(".MediaStatus");
		status_chack_1 = $(status_chack).find('input[type="radio"]:checked').val();



		if(media_campaign == 2 && Template_view == "Multimedia" && status_chack_1 == "publish"){
			$(this).closest('.coupon').find('#show_index').show()
		}else{
			$(this).closest('.coupon').find('#show_index').hide()
			$(this).closest('.coupon').find("#chack_show_data").prop("checked", false);

		}
		console.log(media_campaign,Template_view,status_chack);

	});

	$('.status_chack').on('change', function(e) {

		var media_campaign =$(this).closest('.modal').find("input[type=radio][class=media_campaign_chack]:checked").val();
		let Template_view = $(this).closest('.modal').find("input[name=Template_view]").val();
		status_chack_1 = $(this).val();

		//console.log(media_campaign,Template_view,status_chack_1);
		if(media_campaign == 2 && Template_view == "Multimedia" && status_chack_1 == "publish"){
			$(this).closest('.coupon').find('#show_index').show()
		}else{
			$(this).closest('.coupon').find('#show_index').hide()
	...
	//$('.areaImage').find('input').on('change', function() {
	//	var id = $('.id_ibc').data('idC');
	//	
//
	//});
});
