...
<section class="content-header">
	<h1>{{ trans('article::backend.news_events') }}</h1>
	<ol class="breadcrumb">
    	<li><a href="{{ route('admin.dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('article::backend.home') }}</a></li>
    	<li><a href="{{ route('admin.article.index') }}">{{ trans('article::backend.news_events') }}</a></li>
	</ol>
</section>
					...
					<table id="datatable" class="table table-bordered table-striped">
						...
						<tbody>
							...
							<tr data-entry-id="{{ $item->id }}">
								...
								<td>
									<a href="{{ route('admin.article.edit', $item->id) }}" class="btn btn-primary">
